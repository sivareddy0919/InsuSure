...
const LineGraphExample = ({ route }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${Config.API_BASE_URL}/Doctorlogin.ph/GraphScreen.php`, {
                    params: { username }
                });
                ...
                // Get only the most recent 9 entries
                const recentData = fetchedData.slice(-9);

                const fetchedSugarConcentration = recentData.map(item => parseFloat(item.sugar_concentration));
                const fetchedInsulinIntake = recentData.map(item => `${parseFloat(item.insulinintake)} units`);

                const fetchedLabels = recentData.map(item => {
        ...
        <View style={styles.outerContainer}>
            <ScrollView contentContainerStyle={styles.scrollViewContainer}>
                <View style={styles.topContainer}>
                    ...
                    <Text style={styles.topTitle}>Graph</Text>
                </View>
